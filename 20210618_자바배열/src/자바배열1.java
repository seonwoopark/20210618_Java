 ...
 *    3) 배열 선언 
 *           데이터형[] 배열명; 예) int[] arr; ==> 저장할 수 있는 데이터 유형 (Byte, short, char, int)
 *           	=> 권장 사항      byte[] arr;
 *           					char[] arr;
 *           					long[] arr;
 *           					double[] arr;
 ...
 *    4) 배열 초기화
 *       => 선언과 동시에 배열의 초기화
 *          int[] arr={1,2,3,4,5}; ==> 5개의 데이터를 한번의 저장
 *       => 선언 => 나중에 값을 채운다.
 ...
 *         
 *         일반 for
 *         for(int i=0;i<4;i++) ===> 일반 for (index번호 이용) ==> 배열에 저장된 데이터 변경 , 출력
 *         {
 ...
 *                for(int a:arr)
 *                { ===== 실제 배열에 저장된 데이터 값을 가지고 온다. (인덱스를 사용하지 않는다.)
 *                   
 *                }
 *             
 *    6) 배열 값을 변경
...
//		Runtime.getRuntime().exec("C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe https://sports.news.naver.com/wfootball/index.nhn");
		//1. 배열 선언
		int[] arr= new int[5]; // 연속적으로 메모리에 5개의 정수를 저장한다.
		
		// 2. 데이터를 변경 ==> 일반 for을 이용한다.(배열의 index를 이용한다)
		// length => 배영 선언시 등록된 갯수
